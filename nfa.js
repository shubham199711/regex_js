...
*/
const { toParseTree } = require("./parser");
...
function toNFAfromParseTree(root) {
  ...
  if (root.label === "Factor") {
    ...
    if (root.children.length === 2) {
      // Factor -> Atom MetaChar
      const meta = root.children[1].label;
      if (meta === "*") return closure(atom);
      if (meta === "+") return oneOrMore(atom);
      if (meta === "?") return zeroOrOne(atom);
    }
...
function toNFAFromInfixExp(infixExp) {
  if (infixExp === "") return fromEpsilon();
